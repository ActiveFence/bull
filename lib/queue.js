...
*/
Queue.prototype.add = async function(name, data, opts) {
  if (typeof name !== 'string') {
    opts = data;
    data = name;
    name = Job.DEFAULT_JOB_NAME;
  ...
  if (opts.repeat) {
    if (opts.repeat.constantId) {
      const constantIdKey = utils.getConstantIdKey(opts.repeat.constantId, this.name);
      opts.repeat.constantIdKey = constantIdKey;
      if (await this.client.exists(constantIdKey)){
        return;
      }
    }
