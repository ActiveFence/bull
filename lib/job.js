...
function updateConstantId(client, constantIdKey, jobId, type) {
    ...
    return client.updateConstantId(keys);
}
...
Job.create = function(queue, name, data, opts) {
  ...
  return queue
    ...
    .then(jobId => {
      job.id = jobId;
      if (opts.repeat && opts.repeat.constantIdKey) {
        updateConstantId(queue.client, opts.repeat.constantIdKey, opts.jobId, "add");
      }
...
Job.prototype.moveToCompleted = function(
  ...
  if (this.opts.repeat && this.opts.repeat.constantIdKey) {
    updateConstantId(this.queue.client, this.opts.repeat.constantIdKey, this.opts.jobId, "remove");
  }
