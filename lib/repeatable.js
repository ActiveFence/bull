...
module.exports = function(Queue) {
  Queue.prototype.nextRepeatableJob = function(
    ...
    if (repeat.relativeMills) {
      if (repeat.count === undefined && repeat.offset != 0) {
        nextMillis = now + repeat.offset;
      } else {
        nextMillis = now + repeat.every;
      }
    } else {
      nextMillis = getNextMillis(now, repeat);
    }
  ...
  Queue.prototype.removeRepeatable = async function(name, repeat) {
    ...
    if (repeat.constantId) {
      const constantIdKey = utils.getConstantIdKey(repeat.constantId, this.name);
      const jobReferences = await this.client.smembers(constantIdKey);
      jobReferences.forEach(jobReference => {
        const keys = [
          utils.getRepeatableQueueKey(repeat.queue),
          utils.getRepeatableJobKey(repeat.queue, jobReference),
          constantIdKey,
          jobReference,
        ]
        this.client.removeRepeatableByConstantId(keys);
      })
    }
